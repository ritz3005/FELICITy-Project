#Association between Methylation and FSI: EWAS
...
################DMPs:- FSI##########################################################################################################
##checking variables##
sum(is.na(samps$HRV_FSI))
#[1] 18

##Creating a design model matrix
dim(model.matrix(~ HRV_FSI + Gender  + Smoking + Autoimmunediseases + Gestational_Diabetes + Gestational_age_birth + Illumina.Plate, samps))
#[1] 96 8

design <- model.matrix(~ HRV_FSI  + Gender + Smoking + Autoimmunediseases + Gestational_Diabetes + Gestational_age_birth +Illumina.Plate, samps)
...
##saving the results
write.table(DMPs,paste0(rdir, "/FSI/FSI_FELICITy_DMPs.csv"), sep=",", row.names=FALSE)

save(DMPs, samps,  file = paste0(rdir,"DMPS_FSI_adjustedmod.Rdata"))
#################################################################################################
